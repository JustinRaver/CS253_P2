...
int main(int argc, char **argv) {
  ...
  //if the user inputs a ptter then the code below is executed  
  else{
    //initializes a char array with 1023 slots
    char line[1023];
    //if the user inputs the non case sensitive mode this code is executed
    if(caseSensitive == 0){
      //while fgets has lines to read in this is executed
      while(fgets(line, 1023, stdin) != NULL){
        //if the non case sensitive pattern is found 
        if((strcasestr(line,pattern)) != 0){
          //prints line to stdout 
          puts(line);
          //exits with a normal status
          exitStatus = 0;
        }
      }
    // else the user want the case sensative mode
    }else {
      // while fgets has lines to read in this is executed
      while(fgets(line,1023,stdin) != NULL){
        // if the case sensitive pattern is found prints to std out 
